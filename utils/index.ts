...
export function validURL(str: string) {
  const pattern = /^(https?|ftp):\/\/[^\s/$.?#].[^\s]*$/ // fragment locator
  return pattern.test(str)
}

export const existPassport = (password: string) => {
  return new Promise((resolve, reject) => {
    useApi()
      .$post(`/application/ApplicationValidate/`, {
        body: { passport_serial_number: password },
      })
      .then((res) => {
        resolve(res)
      })
