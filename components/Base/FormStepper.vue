<template>
  <div class="bg-white">
    <div
      id="parent-element"
      ref="parent"
      class="flex items-center px-2 sm:px-6 pt-3 sm:pt-4 pb-3 border-b-[4px] border-b-gray-5 relative before:absolute before:-bottom-1 before:left-0 before:h-1 before:bg-red after:absolute after:-bottom-6 after:left-0 after:h-5"
    >
      <div
        v-for="(item, index) in steps"
        :key="index"
        :data-step="item.step"
        class="w-full relative before:absolute before:w-px before:h-5 before:left-0 before:-bottom-9 before:bg-gray-3 text-xs sm:text-sm font-medium sm:font-semibold text-gray-1"
